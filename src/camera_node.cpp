...
void CamBEV::ImageCallback(const sensor_msgs::ImageConstPtr& msg_img_front, 
    ...
    cv::Mat image_front_left = cv_bridge::toCvShare(msg_img_front_left, "bgra8")->image;
    six_cam_images.push_back(image_front_left);
    cv::Mat image_front = cv_bridge::toCvShare(msg_img_front, "bgra8")->image;
    six_cam_images.push_back(image_front);
    cv::Mat image_front_right = cv_bridge::toCvShare(msg_img_front_right, "bgra8")->image;
    six_cam_images.push_back(image_front_right);
    cv::Mat image_back_right = cv_bridge::toCvShare(msg_img_back_right, "bgra8")->image;
    six_cam_images.push_back(image_back_right);
    cv::Mat image_back = cv_bridge::toCvShare(msg_img_back, "bgra8")->image;
    six_cam_images.push_back(image_back);
    cv::Mat image_back_left= cv_bridge::toCvShare(msg_img_back_left, "bgra8")->image;
    ...
    */
    ...
    sensor_msgs::ImagePtr out_msg = cv_bridge::CvImage(std_msgs::Header(), "bgra8", BEV_image).toImageMsg();
    pub.publish(out_msg);
}
